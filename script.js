    ...
    /**
     * Renderiza o ticker (HTML) em TRÊS pontos: Global (fixo), Landing e Mercado.
    ...
    function renderQuotesTicker() {
        const tracks = [
            ...
            document.getElementById('tickerLandingTrack'),
    ...
    function renderMarketLedger() {
        ...
        list.innerHTML = last5.map(e =>
            `<div style="padding:3px 0; border-bottom:1px solid #111; color:#aaa;">
    ...
    function navigateTo(screenId) {
        ...
        if (screenId === 'engine') { setTimeout(resizeCanvases, 50); renderLandingMarket(); }
    ...
    function logoutSession() {
        ...
        document.getElementById('navMarketBtn').style.display = 'none';
    ...
    function renderMarketGrid() {
        ...
        pageItems.forEach((a) => {
            ...
            if (a.creator !== currentUser.username) {
    ...
    let landingMarketFilter = 'all';
    let landingMarketPage = 0;

    function setLandingMarketFilter(f) {
        landingMarketFilter = f; landingMarketPage = 0;
        document.querySelectorAll('#landingMarketFilterBar .filter-btn').forEach(b => {
            b.className = 'filter-btn' + (b.dataset.filter === f ? (f==='epic'?' active-epic':f==='legendary'?' active-legendary':' active') : '');
        });
        renderLandingMarket();
    }
    ...
    function renderLandingMarket() {
        const grid = document.getElementById('landingMarketGrid'); if (!grid) return;
        grid.innerHTML = '';
        const filtered = landingMarketFilter === 'all' ? marketAssets : marketAssets.filter(a => a.rarityType === landingMarketFilter);
        const pageItems = filtered.slice(landingMarketPage * PAGE_SIZE, (landingMarketPage + 1) * PAGE_SIZE);
        if (filtered.length === 0) {
            grid.innerHTML = '<div class="empty-vault-notice">MERCADO VAZIO // NENHUM ATIVO LISTADO.</div>';
            renderPagination('landingMarketPagination', 0, 0, ()=>{}); return;
        }
        pageItems.forEach(a => {
            const card = document.createElement('div');
            card.className = `album-card rare-${a.rarityType}`;
            card.innerHTML = `
                <div class="album-preview-wrapper"><img src="${a.imgSrc}"></div>
                <div class="album-meta">
                    <div class="album-id">${a.id} <span style="font-size:0.6rem;color:#00ff66;cursor:pointer;" class="lm-ext">by ${a.creator}</span></div>
                    <div class="album-price">${a.price} B$</div>
                </div><div class="card-actions"></div>`;
            card.querySelector('.album-preview-wrapper').addEventListener('click', () => openInspectModal(marketAssets.find(m => m.id === a.id)));
            card.querySelector('.lm-ext').addEventListener('click', (e) => { e.stopPropagation(); viewExternalProfile(a.creator); });
            if (a.creator !== currentUser.username) {
                const buyBtn = document.createElement('button');
                buyBtn.className = 'btn-action'; buyBtn.style.cssText = "background:#ffaa00;color:#000;";
                buyBtn.innerText = currentLang === 'PT' ? 'COMPRAR DIRETO' : 'BUY DIRECT';
                buyBtn.addEventListener('click', () => {
                    if (!currentUser.loggedIn) {
                        showCyberAlert(
                            currentLang === 'PT' ? '🔒 LOGIN NECESSÁRIO' : '🔒 LOGIN REQUIRED',
                            currentLang === 'PT' ? 'Faça login para comprar ativos no mercado.' : 'Login required to purchase assets.',
                            'warn');
                        return;
                    }
                    buyMarketAsset(a.id); renderLandingMarket();
                });
                card.querySelector('.card-actions').appendChild(buyBtn);
            }
            grid.appendChild(card);
        });
        renderPagination('landingMarketPagination', filtered.length, landingMarketPage, (p) => { landingMarketPage = p; renderLandingMarket(); });
    }
